...
TEST(MyDeathTest, Foo) {
  // This death tst uses a compound statement.
  ASSERT_DEATH({
    int n = 5;
    Foo(&n);
...
TEST(MyDeathTest, TestOne) {
  GTEST_FLAG_SET(death_test_style, "threadsafe");
  // This tst is run in the "threadsafe" style:
  ASSERT_DEATH(ThisShouldDie(), "");
}

TEST(MyDeathTest, TestTwo) {
  // This tst is run in the "fast" style:
  ASSERT_DEATH(ThisShouldDie(), "");
}
```
...
TEST(FooTest, Bar) {
  Subroutine();  // The intended behavior is for the fatal failure
                 // in Subroutine() to abort the entire tst.
...
class FooTest : public testing::Test {
 protected:
  // Per-tst-suite set-up.
  // Called before the first tst in this tst suite.
  ...
  // Per-tst-suite tear-down.
  // Called after the last tst in this tst suite.
  // Can be omitted if not needed.
  ...
  // You can define per-tst set-up logic as usual.
  void SetUp() override { ... }

  // You can define per-tst tear-down logic as usual.
...
class FooTest :
  ...
  // To access the tst parameter, call GetParam() from class
  // TestWithParam<T>.
};
...
```c++
  // Gets information about the currently running tst.
  // Do NOT delete the returned object - it's managed by the UnitTest class.
  const testing::TestInfo* const test_info =
      testing::UnitTest::GetInstance()->current_test_info();

  printf("We are in tst %s of tst suite %s.\n",
         test_info->name(),
